...
#%%
smiles_df = pd.read_excel('../tg403_raw_BSY.xlsx')
...
for i in tqdm(range(len(df))):
    try: 
        ...
        try:
            ...
            # admin = list(map(lambda x: remove_bracket(x), admin_tmp.find_next_siblings('h4')))
            # admin_idx = admin.index('Administration / exposure')
            
            # route_tmp = admin_tmp.find_next_siblings('dl')[admin_idx]
            
            # if remove_bracket(route_tmp.find_next('dt')) == 'Route of administration:':
            #     chem_dict['Route of administration'] = remove_bracket(route_tmp.find_next('dd'))
            # else:
