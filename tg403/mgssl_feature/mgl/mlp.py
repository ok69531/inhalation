...
def main(seed_):
    ...
    train_x = tf.cast(train_mgl_features, tf.float32)
    train_y = tf.cast(train_mgl_y, tf.int32)
    test_x = tf.cast(test_mgl_features, tf.float32)
    test_y = tf.cast(test_mgl_y, tf.int32)
    
    
    params_dict = {
        'learning_rate': [0.01, 0.001],
        'epochs': [30, 50, 100, 500, 1000]
    }
...
pd.DataFrame(result).to_csv('../../test_results/features/mgl_mlp.csv', header = True, index = False)
wandb.finish()
